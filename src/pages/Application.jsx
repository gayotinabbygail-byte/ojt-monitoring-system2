...
const applications = [
  ...
  { id: "APP-2026-0041", name: "Juan Dela Cruz", studentId: "2023-BA-0091", course: "Bachelor of Science in Business Administration Major in Financial Management", company: "LCCI Cooperative", date: "Sep 10, 2026", status: "Approved", email: "juan.delacruz@lccian.edu.ph", phone: "0917 555 0091", documents: ["Resume.pdf", "Medical Certificate.pdf"] },
  { id: "APP-2026-0040", name: "Angela Reyes", studentId: "2023-ACC-0068", course: "Bachelor of Science in Business Administration Major in Marketing Management", company: "F. Manalo Accounting", date: "Sep 09, 2026", status: "Under Review", email: "angela.reyes@lccian.edu.ph", phone: "0917 555 0068", documents: ["Resume.pdf", "Transcript of Records.pdf"] },
  { id: "APP-2026-0039", name: "Carlo Mendoza", studentId: "2023-EDU-0143", course: "Bachelor of Science in Elementary Education", company: "Isabela Central School", date: "Sep 08, 2026", status: "Rejected", email: "carlo.mendoza@lccian.edu.ph", phone: "0917 555 0143", documents: ["Resume.pdf"] },
  { id: "APP-2026-0038", name: "Sofia Garcia", studentId: "2023-IT-0212", course: "Bachelor of Science in Information Technology", company: "North Valley Digital", date: "Sep 07, 2026", status: "Approved", email: "sofia.garcia@lccian.edu.ph", phone: "0917 555 0212", documents: ["Resume.pdf", "Endorsement Letter.pdf"] },
  { id: "APP-2026-0037", name: "Paolo Navarro", studentId: "2023-HM-0039", course: "Bachelor of Science in Hospitality Management", company: "Villa Isabela Hotel", date: "Sep 06, 2026", status: "Pending", email: "paolo.navarro@lccian.edu.ph", phone: "0917 555 0039", documents: ["Resume.pdf", "Medical Certificate.pdf"] },
  { id: "APP-2026-0036", name: "Bea Villanueva", studentId: "2023-BA-0117", course: "Bachelor of Science in Business Administration Major in Marketing Management", company: "Provincial Capitol", date: "Sep 05, 2026", status: "Under Review", email: "bea.villanueva@lccian.edu.ph", phone: "0917 555 0117", documents: ["Resume.pdf", "Transcript of Records.pdf"] },
  { id: "APP-2026-0035", name: "Nico Alvarez", studentId: "2023-IT-0151", course: "Bachelor of Science in Information Technology", company: "Pixel North Studio", date: "Sep 04, 2026", status: "Approved", email: "nico.alvarez@lccian.edu.ph", phone: "0917 555 0151", documents: ["Resume.pdf", "Endorsement Letter.pdf"] },
  { id: "APP-2026-0034", name: "Lara Domingo", studentId: "2023-EDU-0088", course: "Bachelor of Science in Secondary Education", company: "LCCI Integrated School", date: "Sep 03, 2026", status: "Pending", email: "lara.domingo@lccian.edu.ph", phone: "0917 555 0088", documents: ["Resume.pdf"] },
  { id: "APP-2026-0033", name: "Miguel Flores", studentId: "2023-ACC-0044", course: "Bachelor of Science in Business Administration Major in Financial Management", company: "Cabreros & Co.", date: "Sep 02, 2026", status: "Rejected", email: "miguel.flores@lccian.edu.ph", phone: "0917 555 0044", documents: ["Resume.pdf", "Transcript of Records.pdf"] },
  { id: "APP-2026-0032", name: "Ella Ramos", studentId: "2023-HM-0102", course: "Bachelor of Science in Hospitality Management", company: "Casa Del Rio", date: "Sep 01, 2026", status: "Pending", email: "ella.ramos@lccian.edu.ph", phone: "0917 555 0102", documents: ["Resume.pdf"] },
];
void applications;
...
function Application() {
  ...
  const [newApplication, setNewApplication] = useState({ name: "", studentId: "", course: "", company: "", email: "", phone: "" });
  const pageSize = 10;

  useEffect(() => onSnapshot(collection(db, "applications"), (snapshot) => {
    setApplicationRecords(snapshot.docs.map((item) => {
      const data = item.data();
      return {
        id: item.id,
        ...data,
        name: data.name || data.studentName || data.student || "Unnamed student",
        studentId: data.studentId || data.studentID || data.idNumber || "Not provided",
        course: data.course || data.program || data.courseProgram || "Not provided",
        company: data.company || data.partnerCompany || data.companyName || "Not assigned",
  ...
  const counts = applicationRecords.reduce((summary, application) => {
  ...
  const updateStatus = (application, status) => {
    updateDoc(doc(db, "applications", application.id), { status, updatedAt: serverTimestamp() })
      .then(() => showNotice(`${application.name}'s application is marked ${status.toLowerCase()}.`))
      .catch((statusError) => {
  ...
  const saveNewApplication = async (event) => {
    ...
    setFormError("");
    if (!newApplication.name.trim() || !newApplication.studentId.trim() || !newApplication.course || !newApplication.company.trim()) {
      setFormError("Student name, student ID, course, and company are required.");
      return;
    }
    setSaving(true);
    try {
      const record = { ...newApplication, name: newApplication.name.trim(), studentName: newApplication.name.trim(), studentId: newApplication.studentId.trim(), company: newApplication.company.trim(), email: newApplication.email.trim(), phone: newApplication.phone.trim(), date: new Date().toISOString().slice(0, 10), status: "Pending", createdAt: serverTimestamp() };
      const reference = await addDoc(collection(db, "applications"), record);
      setApplicationRecords((current) => [{ ...record, id: reference.id, createdAt: new Date().toISOString() }, ...current]);
      setNewApplication({ name: "", studentId: "", course: "", company: "", email: "", phone: "" });
  ...
  return (
    <main className="application-page">
      ...
      <header className="application-header">
        ...
        <button type="button" className="application-primary" onClick={() => { setFormError(""); setShowNewApplication(true); }}><Plus size={17} /> New Application</button>
      </header>

      <section className="application-summary" aria-label="Application summary">
        <article className="application-summary-card"><span className="summary-icon summary-blue"><ClipboardList size={18} /></span><div><strong>{applicationRecords.length}</strong><span>Total Applications</span></div></article>
      ...
      <section className="application-panel">
        {loadError && <div className="application-form-error" role="alert">{loadError}</div>}
        {loading && <div className="empty-applications">Loading applications...</div>}
      ...
      {showNewApplication && <div className="application-modal-backdrop" role="presentation" onClick={(event) => { if (event.target === event.currentTarget && !saving) setShowNewApplication(false); }}><section className="application-modal" role="dialog" aria-modal="true" aria-labelledby="new-application-title"><header className="application-modal-header"><div><h2 id="new-application-title">New Application</h2><p>Submit a student OJT application for review.</p></div><button type="button" className="application-modal-close" onClick={() => setShowNewApplication(false)} aria-label="Close new application form"><X size={17} /></button></header><form className="application-detail-grid" onSubmit={saveNewApplication}>{formError && <div className="application-form-error application-detail full" role="alert">{formError}</div>}<label className="application-detail"><span>Student Name</span><input value={newApplication.name} onChange={(event) => setNewApplication({ ...newApplication, name: event.target.value })} required /></label><label className="application-detail"><span>Student ID</span><input value={newApplication.studentId} onChange={(event) => setNewApplication({ ...newApplication, studentId: event.target.value })} required /></label><label className="application-detail"><span>Course / Program</span><select value={newApplication.course} onChange={(event) => setNewApplication({ ...newApplication, course: event.target.value })} required><option value="">Select course</option>{courses.map((course) => <option key={course} value={course}>{course}</option>)}</select></label><label className="application-detail"><span>Partner Company</span><input value={newApplication.company} onChange={(event) => setNewApplication({ ...newApplication, company: event.target.value })} required /></label><label className="application-detail"><span>Email</span><input type="email" value={newApplication.email} onChange={(event) => setNewApplication({ ...newApplication, email: event.target.value })} /></label><label className="application-detail"><span>Phone</span><input value={newApplication.phone} onChange={(event) => setNewApplication({ ...newApplication, phone: event.target.value })} /></label><div className="application-form-actions application-detail full"><button type="button" className="application-action" onClick={() => setShowNewApplication(false)} disabled={saving}>Cancel</button><button type="submit" className="application-primary" disabled={saving}>{saving ? "Saving..." : "Submit Application"}</button></div></form></section></div>}
