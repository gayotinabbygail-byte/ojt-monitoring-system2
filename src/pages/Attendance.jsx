    ...
    () =>
      records.filter((record) => {
        ...
        return (
          record.date === date &&
          (!query ||
            `${record.student} ${record.studentId} ${record.company}`
              .toLowerCase()
              .includes(query)) &&
  ...
  return (
    <main className="attendance-page">
      <header className="attendance-header">
        <div>
          <p className="attendance-eyebrow">Daily monitoring</p>
          <h1>Attendance</h1>
          <p>
            Review student time logs, flag exceptions, and keep placement hours
            accurate.
          </p>
        </div>
        <button
          ...
          onClick={() =>
            showNotice("Attendance report export is ready to connect.")
          }
        >
          <Download size={16} /> Export report
        </button>
      </header>
      <section className="attendance-context" aria-label="Attendance context">
        <div className="context-icon">
          <CalendarDays size={20} />
        </div>
        <div>
          <span>Viewing attendance for</span>
          <strong>
      ...
      {error ? (
        <section className="attendance-panel">
          <div className="attendance-empty" role="alert">
            <strong>{error}</strong>
            <button type="button" className="attendance-export" onClick={retry}>
              Retry
            </button>
          </div>
        </section>
        ...
        <section className="attendance-panel">
          <div className="attendance-empty">Loading attendance records...</div>
        </section>
      ) : (
        <section className="attendance-panel">
              ...
              <tbody>
                {visibleRecords.map((record) => (
                  <tr key={record.id}>
                    ...
                    <td>
                      <span
                        className={`attendance-status ${statusClass[record.status]}`}
                      >
                        {record.status}
                      </span>
                    </td>
                    <td>
                      <button
                        ...
                        aria-label={`View ${record.student} attendance`}
          ...
          <footer className="attendance-footer">
            Showing {visibleRecords.length} student records for the selected
            date
          </footer>
        </section>
      )}
      {selected && (
        ...
        >
          ...
          >
            <header>
              <div>
                <p className="attendance-eyebrow">Attendance record</p>
                <h2 id="attendance-modal-title">{selected.student}</h2>
                <span>
                  {selected.studentId} · {selected.company}
                </span>
              </div>
              <button
                type="button"
            ...
            <div className="attendance-detail-grid">
              <div>
                <span>Time in</span>
                <strong>{selected.timeIn}</strong>
              </div>
              <div>
                <span>Time out</span>
                <strong>{selected.timeOut}</strong>
              </div>
              <div>
                <span>Logged hours</span>
                <strong>{selected.hours} hours</strong>
              </div>
              <div>
                <span>Note</span>
            ...
            <div className="attendance-modal-actions">
              <button type="button" onClick={() => verifyRecord(selected)}>
                Mark verified
              </button>
              <button
                type="button"
